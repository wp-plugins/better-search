...
function bsearch_template_redirect() {
	...
	$exists = file_exists(ALD_BSEARCH_DIR . '/better-search-template.php');
	if ($exists)
	{
		//include_once(get_bloginfo('stylesheet_directory') . '/better-search-template.php');
		include_once(ALD_BSEARCH_DIR . '/better-search-template.php');
		exit;
	}
...
function bsearch_plugin_actions( $links, $file ) {
	...
	if ($file == $plugin) {
		...
		$links[] = '<a href="http://ajaydsouza.org">' . __('Support', BSEARCH_LOCAL_NAME ) . '</a>';
